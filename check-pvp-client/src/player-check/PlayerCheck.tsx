...
const PlayerCheck: React.FunctionComponent<{}> = (props: any) => {
  ...
  const existingStoreCharacterId = characterIds.find(id => isEqualIgnoringCase(id, `${name}-${realm}-${region}`));
  ...
  return (
    <Flex justifyBetween margin="30px 10%">
      <Col>
        ...
        <PlayerSummary character={characters && existingStoreCharacterId ? characters[existingStoreCharacterId] : undefined} />
      </Col>
      <Col>
        <RecentCheck characterClick={character => handleSearch(`${character.name}-${character.realm}`)} />
      </Col>
    </Flex>
  );
};
